...
import {
  ...
  ACTUAL_X, ACTUAL_Y, METHOD_POST, AUTH_LABEL,
  SFX_MAP,
} from './data'
...
export default function CorsAnimation({
  ...
  useEffect(() => {
    ...
    t += 1.6
    // guardrail teknis (revisi-02 §3/§5): wildcard Access-Control-Allow-Origin
    // tidak kompatibel dengan Allow-Credentials true — label singkat, tanpa
    // state/visual baru, hanya caption tambahan sebelum konfirmasi match.
    say(t, CAPTIONS.WILDCARD_GUARD)
    t += 1.3
  ...
  // (revisi-04: helper `OriginCard` dead code dihapus — didefinisikan tapi
  // tidak pernah dipakai di JSX; app.example/api.example dirender langsung
  // inline di bawah, bukan lewat komponen ini.)
  ...
  return (
    <svg ref={svgRef} viewBox={`0 0 ${VW} ${VH}`}
      ...
      {/* ── HEADER (revisi-04: gate pakai headerOpacity > 0, bukan
          contentStarted — contentStarted baru true SETELAH tween opacity
          selesai, jadi fade morph hero→header sebelumnya tidak pernah
          kelihatan, header cuma "muncul tiba-tiba" penuh. Pola ini sama
          dengan referensi 23-https-tls.) ── */}
      {headerOpacity > 0 && (
      ...
      {/* ── BODY — ContentBodyV1 langsung, TANPA wrapper T()/O() ekstra.
          (revisi-04: wrapper lama `T('contentBody',0,0)` + `O('contentBody')`
          tidak pernah di-popIn, jadi permanen scale(0)/opacity(0) — seluruh
          body tidak pernah terlihat. Referensi 23-https-tls render
          ContentBodyV1 langsung tanpa wrapper semacam ini.) ── */}
      {contentStarted && (
        <ContentBodyV1 debugName="cors-body">
          ...
          {/* ── actual request (revisi-04: sebelumnya di-popIn tapi tidak
              pernah dirender — beat "request ditahan/berangkat" tidak
              punya visual sama sekali). Ditahan di samping gate (ACTUAL_X),
              lalu versi "berangkat" di posisi sama setelah izin cocok. ── */}
          {actualHeld && (
            <g transform={T('actualCard', ACTUAL_X, ACTUAL_Y)} opacity={O('actualCard')} filter="url(#shadow)">
              <rect x={-84} y={-28} width={168} height={56} rx={10} fill={COLORS.PANEL} stroke={COLORS.DENY} strokeWidth={2} strokeDasharray="4 4" />
              <text x={0} y={-8} textAnchor="middle" fontSize={9} fontWeight={900} fontFamily="monospace" fill={COLORS.DENY}>{METHOD_POST}</text>
              <text x={0} y={10} textAnchor="middle" fontSize={7} fontFamily="monospace" fill={COLORS.MUTED}>{AUTH_LABEL}</text>
            </g>
          )}
          {actualGo && (
            <g transform={T('actualGoCard', ACTUAL_X, ACTUAL_Y)} opacity={O('actualGoCard')} filter="url(#shadow)">
              <rect x={-84} y={-28} width={168} height={56} rx={10} fill={COLORS.PANEL} stroke={COLORS.ALLOW} strokeWidth={2} />
              <text x={0} y={-8} textAnchor="middle" fontSize={9} fontWeight={900} fontFamily="monospace" fill={COLORS.ALLOW}>{METHOD_POST}</text>
              <text x={0} y={10} textAnchor="middle" fontSize={7} fontFamily="monospace" fill={COLORS.MUTED}>{AUTH_LABEL}</text>
            </g>
          )}
          ...
          {/* ── CAPTION (revisi-04: pakai CAPTION_Y, sebelumnya CLOSING_Y+130
              yang menghasilkan canvas Y jauh di luar viewBox) ── */}
          {caption && contentStarted && (
            <g transform={`translate(${AXIS_X}, ${CAPTION_Y})`}>
