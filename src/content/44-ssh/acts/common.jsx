// src/content/44-ssh/acts/common.jsx
// ─────────────────────────────────────────────────────────────
...
//   • Satu prop `state = { pop, stage, trustBadgeOn, tunnelActive, channelDim,
//     verifiedOn, keyStatus, allowShown, denyShown, alertOn }`.
//   • Dipanggil TANPA props → mode "summary": semua elemen act dirender di
//     posisi akhir (SUMMARY_POSITIONS) dengan stage akhir (SUMMARY_STAGE) —
//     dipakai intro background/thumbnail.
//   • Prop `origin = {x, y}` (default {0,0}) translate wrapper untuk
//     memposisikan ulang scene (mis. intro bg pakai layout.body; live di dalam
//     ContentBodyV1 cukup {0,0} karena body sudah translate sendiri).
//
// Semua angka koordinat TETAP body-local seperti blok asli di Animation.jsx
// (pilihan "prop origin, nomor tetap"). Duplikat kecil helper pose/transform
// yang sama dari Animation.jsx (pola repo: tidak diimport dari luar folder).
// ─────────────────────────────────────────────────────────────
...
/** Ambil pose (posisi/opacity) elemen dari pop state — default sembunyi. */
...
/** Ops setara A() di Animation: posisi ABSOLUT ada di pop. */
...
export const getChannelStyle = (state = {}) => {
  const active = state.tunnelActive ?? true
  const dim = state.channelDim ?? false
  return {
    color: active ? COLORS.TUNNEL : COLORS.BORDER,
    opacity: active ? (dim ? 0.35 : 1) : 0.5,
    width: active ? 5 : 2,
    dash: active ? undefined : '6 8',
    filter: active && !dim ? 'url(#glow)' : undefined,
  }
}

/**
 * Chrome persisten lintas-act: channel line + client/server anchor +
 * host label. Di-summary (tanpa state) channel tampil aktif emerald dan
 * trust badge menyala → scene masih "hidup" di thumbnail.
 */
export function ActChrome({ state }) {
  const css = getChannelStyle(state)
  const trustBadgeOn = state?.trustBadgeOn ?? true
  return (
    <>
