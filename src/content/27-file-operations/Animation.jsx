...
// Menambah Act 5 (grep): grep <kata> <file>, grep -i, grep -r. GrepPreviewPanel
// dan GrepResultRail baru; CommandPulse scan digeneralisasi agar bisa dipakai
// ulang untuk grep -r selain find.
// STATUS: entry aktif sejak 2026-09-16 (menggantikan revisi-04).
// Backup revisi-04: Animation.revisi-04.jsx / data.revisi-04.js
...
const CommandPulse = ({ pulse }) => {
  ...
  if (pulse.kind === 'scan') {
    ...
    const scanLabel = pulse.scanLabel || 'memindai folder nyata'
    const foundLabel = pulse.foundLabel || 'ditemukan: ./assets/banner.png'
    return (
      <g>
        <text x="366" y={ZONE.MOTION.yStart + 14} textAnchor="middle" fontFamily="monospace" fontSize="11" fill={COLORS.MUTED}>{scanLabel}</text>
...
// ── Panel preview grep (Act 5) — mirip FilePreviewPanel, tapi baris konteks
// tetap redup dan hanya baris cocok (`matchIndices`) yang menyala + keyword
// di-highlight. `revealed=false` = Before (redup, belum ada highlight),
// `revealed=true` = After (match menyala) sesuai Causal Motion Contract. ──
const GrepPreviewPanel = ({ grep }) => {
  if (!grep || !grep.visible || !grep.scenario) return null
  const { scenario, revealed } = grep
  const { filename, keyword, modifier, lines, matchIndices } = scenario
  const panelX = 40
  const panelY = ZONE.GRID.yStart + 6
  const panelW = 652
  const rowH = 20
  ...
  const bodyRows = Math.max(lines.length, 1)
  const panelH = headerH + 16 + bodyRows * rowH + 12
  const modeKey = keyword === 'TODO' ? 'grep-recursive' : (modifier ? 'grep-icase' : 'grep-port')
  const verb = GUI_VERB[modeKey]
  const matchSet = new Set(matchIndices || [])

  const renderLine = (line, i) => {
    const isMatch = revealed && matchSet.has(i)
    const y = headerH + 24 + i * rowH
    if (!isMatch || !keyword) {
      return (
        <text key={i} x="20" y={y} fontFamily="monospace" fontSize="12" fill={COLORS.TEXT} opacity={revealed ? 0.4 : 0.55}>{line}</text>
      )
    }
    const idx = line.toLowerCase().indexOf(keyword.toLowerCase())
    if (idx === -1) {
      return <text key={i} x="20" y={y} fontFamily="monospace" fontSize="12" fill={COLORS.TEXT}>{line}</text>
    }
    const before = line.slice(0, idx)
    const match = line.slice(idx, idx + keyword.length)
    const after = line.slice(idx + keyword.length)
    const charW = 7.25
    const hlX = 20 + before.length * charW - 2
    const hlW = match.length * charW + 4
    return (
      <g key={i}>
        <rect x={hlX} y={y - 13} width={hlW} height="17" rx="3" fill={COLORS.FIND} opacity="0.32" />
        <text x="20" y={y} fontFamily="monospace" fontSize="12" fill={COLORS.TEXT}>
          {before}<tspan fill={COLORS.FIND} fontWeight="700">{match}</tspan>{after}
        </text>
      </g>
    )
  }
  ...
  return (
    <g transform={'translate(' + panelX + ' ' + panelY + ')'}>
      <rect x="0" y="0" width={panelW} height={panelH} rx="14" fill={COLORS.PANEL} stroke={COLORS.FIND} strokeWidth="2" />
      <rect x="0" y="0" width={panelW} height={headerH} rx="14" fill={COLORS.PANEL_ALT} />
      <rect x="0" y={headerH - 14} width={panelW} height="14" fill={COLORS.PANEL_ALT} />
      <circle cx="18" cy={headerH / 2} r="5" fill={COLORS.DANGER} />
      <circle cx="34" cy={headerH / 2} r="5" fill={COLORS.CREATE} />
      <circle cx="50" cy={headerH / 2} r="5" fill={COLORS.SUCCESS} />
      <text x={panelW / 2} y={headerH / 2 + 4} textAnchor="middle" fontFamily="monospace" fontWeight="700" fontSize="11.5" fill={COLORS.TEXT}>
        {filename + ' — ' + (verb ? verb.text : 'grep')}
      </text>
      {modifier && (
        <g transform={'translate(' + (panelW - 34) + ' ' + (headerH / 2) + ')'}>
          <rect x="-20" y="-10" width="40" height="20" rx="10" fill={COLORS.PANEL} stroke={COLORS.FIND} strokeWidth="1.3" />
          <text x="0" y="4" textAnchor="middle" fontFamily="monospace" fontWeight="700" fontSize="10" fill={COLORS.FIND}>{modifier}</text>
        </g>
      )}
...
// ── Result rail untuk `grep -r` — kumpulan path yang cocok, bertambah satu
// per satu seiring scan menemukan match (bukan langsung muncul semua). ──
const GrepResultRail = ({ items }) => {
  if (!items || items.length === 0) return null
  const startY = ZONE.MOTION.yEnd + 8
  return (
    <g>
      {items.map((text, i) => (
        <g key={i} transform={'translate(366 ' + (startY + i * 20) + ')'}>
          <rect x="-270" y="-12" width="540" height="20" rx="8" fill={COLORS.PANEL} stroke={COLORS.FIND} strokeWidth="1.2" />
          <text x="0" y="4" textAnchor="middle" fontFamily="monospace" fontSize="10.5" fill={COLORS.TEXT}>{text}</text>
        </g>
      ))}
    </g>
  )
...
export default function FileOperationsAnimation({
  ...
  const [grepPreview, setGrepPreview] = useState({ visible: false, scenario: null, revealed: false })
  const [grepRail, setGrepRail] = useState([])
  ...
  useEffect(() => {
    ...
    // Panel grep (Act 5): tampil redup saat token mulai travel (Before),
    // highlight match baru menyala tepat saat Apply (After) — sesuai Causal
    // Motion Contract, highlight tidak boleh muncul sebelum target tersentuh.
    const scheduleGrepPreview = (step, travelAt, applyAt) => {
      const scenarioKey = step.gui === 'grep-port' ? 'port' : 'ignoreCase'
      const scenario = GREP_SCENARIOS[scenarioKey]
      if (!scenario) return
      const holdExtra = step.holdExtra || 0.8

      tl.add(() => setGrepPreview({ visible: true, scenario, revealed: false }), travelAt)
      tl.add(() => {
        setGrepPreview((prev) => (prev.visible ? { ...prev, revealed: true } : prev))
        play(scenarioKey === 'port' ? SFX_MAP.COPY : SFX_MAP.CHIME)
      }, applyAt)
      tl.add(() => setGrepPreview((prev) => ({ ...prev, visible: false })), applyAt + holdExtra + 0.15)
    }
    ...
    const applyGui = (event) => {
      switch (event) {
        ...
        case 'grep-recursive': play(SFX_MAP.SAVE); break
        default: break
      }
    }
    ...
    tl.add(() => {
      ...
      setGrepPreview({ visible: false, scenario: null, revealed: false })
      setGrepRail([])
    ...
    const actStart = [0, 0, 0, 0, 0]
    actStart[0] = INTRO_DELAY + 0.3
    for (let i = 1; i < 5; i += 1) actStart[i] = actStart[i - 1] + PHASES[i - 1].duration

    const scheduleAct = (actIdx, startAt) => {
      let cursor = startAt + 0.2
      tl.add(() => setPhaseIdx(actIdx), startAt)
      if (actIdx === 4) tl.add(() => setCaption(CAPTIONS.GREP_BRIDGE), startAt + 0.05)
      ...
      steps.forEach((step) => {
        ...
        if (step.full && step.kind === 'cmd') {
          ...
          const isScanGui = step.gui === 'find' || step.gui === 'grep-recursive'
          tl.add(() => {
            if (step.gui !== 'find' && step.gui !== 'grep-recursive') setFocusTarget(step.target || [])
            markTravelState(step.gui)
            const scanLabel = step.gui === 'grep-recursive' ? 'grep -r menyusuri folder' : undefined
            const foundLabel = step.gui === 'grep-recursive' ? (GREP_SCENARIOS.recursive.results.length + ' hasil ditemukan') : undefined
            setPulse({ active: true, kind: isScanGui ? 'scan' : 'dot', progress: 0, verb: (verb && verb.text) || '', colorKey: verbColorKey, scanLabel, foundLabel })
          }, travelAt)
          ...
          } else if (step.gui === 'grep-recursive') {
            const seq = GREP_SCENARIOS.recursive.scanIds
            const matchIds = GREP_SCENARIOS.recursive.matchIds
            const results = GREP_SCENARIOS.recursive.results
            const stepDur = TRAVEL_DUR / seq.length
            seq.forEach((id, i) => {
              tl.add(() => {
                setFocusTarget([id])
                ...
                if (matchIdx !== -1) {
                  play(SFX_MAP.CHIME)
                  setGrepRail((prev) => (prev.includes(results[matchIdx]) ? prev : [...prev, results[matchIdx]]))
                }
              }, travelAt + i * stepDur)
            })
          }
          ...
          } else if (step.gui === 'grep-port' || step.gui === 'grep-icase') {
            scheduleGrepPreview(step, travelAt, applyAt)
          } else {
    ...
    scheduleAct(3, actStart[3])
    const endAct4 = scheduleAct(4, actStart[4])

    tl.add(() => setCaption(CAPTIONS.GREP_CLOSING), endAct4 - 0.4)
    tl.add(() => setCaption(CAPTIONS.TAKEAWAY), endAct4 + 0.3)
    tl.to({}, { duration: 0.01 }, endAct4 + 2.6)
        ...
        <ContentBodyV1 debugName="file-operations-body" clip>
          <g opacity={bodyOpacity}>
            ...
            <g>
              ...
              <GrepPreviewPanel grep={grepPreview} />
            </g>
