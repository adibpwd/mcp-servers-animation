...
> **Tujuan:** Membuat intro bergaya *hacker typing* sebelum Act 1 dimulai, agar penonton langsung tahu topik video.
>
> **Target durasi:** ±5,9–7,6 detik, tergantung cursor blink dan fade transition.

## 📌 RINGKASAN IMPLEMENTASI

| Bagian | Isi |
|---|---|
| Intro | Hacker typing sebelum Act 1 |
| Title | `UNIX vs LINUX` |
| Subtitle | `Perjalanan 1969 → 2026:` + `Dari Bell Labs ke Dunia Digital` |
| Audio | Mechanical keyboard typing + cursor beep + whoosh |
| State | `showIntro`, `introOpacity`, `typingText`, `cursorVisible` |
| Helper | `typeOutText()` |
| Target | ±6–8 detik sebelum Act 1 |
...
| **TOTAL TYPING** | **~5.04s** | Belum termasuk cursor blink + fade |
...
// Total intro + cursor + fade = ~7.6 detik
```
...
**Catatan:** Karena `typingText` menggabungkan seluruh line, index yang dipakai harus konsisten dengan panjang string masing-masing line. Disarankan menyimpan `line1`, `line2`, dan `line3` terpisah lalu merender berdasarkan fase typing agar tidak bergantung pada angka magic seperti `14` dan `40`.
...
Ada beberapa detail yang sebaiknya dibereskan saat implementasi:

1. **Color per karakter pada Line 1:** callback `onChar()` saat ini hanya mengubah variabel `color`; untuk SVG `<text>`, lebih aman render `UNIX`, ` vs `, dan `LINUX` sebagai `<tspan>` terpisah.
2. **`fullText` di JSX:** jangan mengandalkan `typingText.length < fullText.length` jika `fullText` belum didefinisikan di scope JSX. Gunakan konstanta `FULL_TEXT` atau, lebih baik, state/fase per line.
3. **Durasi:** angka pada bagian *Timing Breakdown* adalah acuan yang lebih detail daripada target awal 5–7 detik.
4. **Random delay:** hasil total typing akan sedikit berubah karena delay acak. Jika durasi video harus deterministic, gunakan seeded/randomized delay yang dikontrol.
5. **Audio:** pastikan audio sudah *unlocked* setelah user interaction sebelum timeline memainkan SFX.
